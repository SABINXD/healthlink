...
// Approve doctor verification with notes
// Approve doctor verification with notes
if (isset($_GET['approve_doctor']) && isset($_POST['request_id'])) {
    ...
    if (approveDoctorVerification($request_id, $notes)) {
        $_SESSION['error'] = [
...
// Decline doctor verification with notes
if (isset($_GET['decline_doctor']) && isset($_POST['request_id'])) {
    $request_id = (int)$_POST['request_id'];
    $notes = isset($_POST['notes']) ? mysqli_real_escape_string($db, $_POST['notes']) : '';

    if (declineDoctorVerification($request_id, $notes)) {
        $_SESSION['error'] = [
            "field" => "doctor_verification",
