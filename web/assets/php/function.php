...
function showError($field)
{
    if (isset($_SESSION['error'])) {
        ...
        function isDoctorAvailable($doctorId)
        {
            ...
            $query = "SELECT u.*, dv.hospital_name, dv.department, dv.hospital_address, 
                     dv.hospital_city, dv.years_of_experience, dv.specialty,
                     dv.nmc_number, dv.registration_number, dv.consultation_fee
              FROM users u
              LEFT JOIN doctor_verification dv ON u.id = dv.user_id
              WHERE u.id = $doctorId 
              AND u.is_doctor = 1 
              AND u.ac_status = 1 
              AND dv.status = 1
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                return mysqli_fetch_assoc($result); // return doctor row with verification details
            }
        ...
        // Function for validating doctor verification form (new version)
        function validateVerifyDoctorForm($form_data, $file_data)
        {
            ...
            // File validation
            $files = [
                'certFront' => ['name' => 'Medical Certificate Front', 'required' => true],
                'certBack' => ['name' => 'Medical Certificate Back', 'required' => true],
                'nmcCert' => ['name' => 'NMC Certificate', 'required' => true],
                'citizenship' => ['name' => 'Citizenship Document', 'required' => true]
            ];

            foreach ($files as $fileKey => $fileInfo) {
                if (!isset($file_data[$fileKey]) || !$file_data[$fileKey]['name']) {
                    if ($fileInfo['required']) {
                        $response['status'] = false;
                        $response['msg'] = $fileInfo['name'] . " is required";
                        $response['field'] = $fileKey;
                        return $response;
                    }
                } else {
                    $image = $file_data[$fileKey]['name'];
                    $type = strtolower(pathinfo($image, PATHINFO_EXTENSION));
                    $sizeKB = $file_data[$fileKey]['size'] / 1024; // KB

                    // Allowed file types: jpg, jpeg, png, pdf
                    if (!in_array($type, ['jpg', 'jpeg', 'png', 'pdf'])) {
                        $response['status'] = false;
                        $response['msg'] = "Only jpg, jpeg, png, pdf allowed for " . $fileInfo['name'];
                        $response['field'] = $fileKey;
                        return $response;
                    }
                    ...
                    if ($sizeKB > 5120) {
                        $response['status'] = false;
                        $response['msg'] = "File size must be less than 5MB for " . $fileInfo['name'];
                        $response['field'] = $fileKey;
                        return $response;
                    }
                }
        ...
        function insertVerifyDoctor($data, $imagedata)
        {
            ...
            // Upload directory - FIXED: Match the path used in the admin panel
            $uploadDir = "../assets/img/verifydoctor/";
            if (!is_dir($uploadDir)) {
            ...
            // Handle files
            $files = ['certFront', 'certBack', 'nmcCert', 'citizenship'];
            $uploadedFiles = [];
            foreach ($files as $file) {
                $uploadedFiles[$file] = '';
                if (isset($imagedata[$file]) && $imagedata[$file]['name']) {
                    // Generate a unique filename
                    $fileExtension = pathinfo($imagedata[$file]['name'], PATHINFO_EXTENSION);
                    $image_name = time() . '_' . uniqid() . '.' . $fileExtension;
                    $image_path = $uploadDir . $image_name;

                    if (move_uploaded_file($imagedata[$file]['tmp_name'], $image_path)) {
                        $uploadedFiles[$file] = $image_name;
                        error_log("Successfully uploaded file: $file to $image_path");
                    } else {
                        error_log("Failed to upload file: " . $file);
                        return "Failed to upload file: $file";
                    }
                }
            }

            // Format dates to YYYY-MM-DD for MySQL - FIXED
            $dateOfBirth = !empty($data['dateOfBirth']) ? date('Y-m-d', strtotime($data['dateOfBirth'])) : null;
            $nmcIssueDate = !empty($data['nmcIssueDate']) ? date('Y-m-d', strtotime($data['nmcIssueDate'])) : null;
            $nmcExpiryDate = !empty($data['nmcExpiryDate']) ? date('Y-m-d', strtotime($data['nmcExpiryDate'])) : null;

            // Use prepared statement to prevent SQL injection
            $query = "INSERT INTO doctor_verification (
        user_id, full_name, other_name, date_of_birth, gender, nationality, languages,
        doctor_type, specialty, years_of_experience, medical_school, graduation_year, registration_number,
        nmc_number, nmc_issue_date, nmc_expiry_date, nmc_status,
        hospital_name, department, hospital_address, hospital_city, hospital_phone, hospital_email,
        cert_front, cert_back, nmc_cert, citizenship,
        email, phone, address, status
            ...
            // Set status as integer (0 for pending)
            $status = 0;

            // Bind parameters
            mysqli_stmt_bind_param(
                $stmt,
                "issssssssissssssssssssssssssi",   // Last parameter is integer (i)
                $user_id,
                $data['fullName'],
                $data['otherName'],
                $dateOfBirth,
                ...
                $data['hospitalPhone'],
                $data['hospitalEmail'],
                $uploadedFiles['certFront'],
                $uploadedFiles['certBack'],
                $uploadedFiles['nmcCert'],
                $uploadedFiles['citizenship'],
                $data['email'],
                $data['phone'],
                $data['address'],
                $status   // This is now an integer (0)
            );

            // Execute the statement
            if (mysqli_stmt_execute($stmt)) {
                $insert_id = mysqli_insert_id($db);
                error_log("Successfully inserted doctor verification with ID: $insert_id");
...
Caption: $caption"
    ...
    ";
        ...
        function deductCredits($user_id, $amount)
        {
            global $db;
            $user_id = (int)$user_id;
            $amount = (int)$amount;
            ...
            $checkQuery = "SELECT credits FROM users WHERE id = $user_id";
            $result = mysqli_query($db, $checkQuery);
            ...
            if ($result) {
                ...
                return [
                    'success' => false,
                    'message' => 'Failed to deduct credits: ' . mysqli_error($db)
                ];
            }
        }
